Feature: LimitationsNumberStockSymbol
        By Admin 
...
Scenario:01) time limit for advertising the stock symbol
	...
	When  Go to Menu limitations And Click Edit limitation insert Number from 0 to 2
	Then The result Should be insert Time 
	Examples:
	| Title |Symbolname| Massagefarsi |
	| معامله با تعداد مشخص شده ،به دلیل محدودیت  در بازار اعمال نمی شود | خساپا| اعمال محدودیت تعداد در نماد خساپا|
...
Scenario:  02)Buy stock symbol  with a minimum amount and a minimum number of day
	...
	Then the result should Buy shares of the symbol Message <Massagefarsi>
		Examples:
	 | Massagefarsi                      |
	 | اعمال محدودیت تعداد در نماد خساپا |
