Feature: LimitationsCustomerStockSymbol
	By Admin 

	@GoToUrlAndLoadPageAdmin@LogoutAdmin
Scenario:01) Customer limit for advertising the stock symbol
	...
	When  Go to Menu limitations And Click Edit limitation insert Customer Name مجید یعقوبی
	Then The result Should be insert Time 
	Examples:
	| Title |Symbolname| Massagefarsi |
	| معامله با تعداد مشخص شده ،به دلیل محدودیت  در بازار اعمال نمی شود | خساپا| اعمال محدودیت مشتری در نماد خساپا|
...
Scenario:  02)Buy stock symbol  with a minimum amount and a minimum number of day
	...
	Then the result should Buy shares of the symbol Message <Massagefarsi>
		Examples:
	 | Massagefarsi                      |
	 | اعمال محدودیت مشتری در نماد خساپا |
