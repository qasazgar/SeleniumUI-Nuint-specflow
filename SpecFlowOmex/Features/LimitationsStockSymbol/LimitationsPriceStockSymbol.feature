Feature: LimitationsPriceStockSymbol
...
Scenario:01) time limit for advertising the stock symbol
	...
	When  Go to Menu limitations And Click Edit limitation insert Price from 1000 to 5000
	Then The result Should be insert Time 
	Examples:
	| Title |Symbolname| Massagefarsi |
	| معامله با قیمت مشخص شده ،به دلیل محدودیت  در بازار اعمال نمی شود | خساپا| اعمال محدودیت قیمت در نماد خساپا|
...
Scenario:  02)Buy stock symbol  with a minimum amount and a minimum number of day
	...
	Then the result should Buy shares of the symbol Message <Massagefarsi>
		Examples:
	 | Massagefarsi                      |
	 | اعمال محدودیت قیمت در نماد خساپا |
