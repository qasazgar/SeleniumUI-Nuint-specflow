Feature: LimitationsTimeValidityTypeStockSymbol
	By Admin 

	@GoToUrlAndLoadPageAdmin@LogoutAdmin
Scenario:01)Time Validity Type limit for advertising the stock symbol
	...
	When  Select the روز value in the  Time Validity Type search box  
	Then The result Should be insert Time 
	Examples:
	| Title |Symbolname| Massagefarsi |
	| معامله با تعداد مشخص شده ،به دلیل نوع اعتبار زمانی معامله در بازار اعمال نمی شود | خساپا| اعمال محدودیت نوع اعتبار زمانی معامله در نماد خساپا|
...
Scenario:  02)Buy stock symbol  with a minimum amount and a minimum number of day
	...
	Then the result should Buy shares of the symbol Message <Massagefarsi>
		Examples:
	 | Massagefarsi                      |
	 | اعمال محدودیت نوع اعتبار زمانی معامله در نماد خساپا |
