Feature: LimitationsTransactionTypeStockSymbol
	By Admin 
...
Scenario:01)Transaction Type limit for advertising the stock symbol
	...
	When  Go to Menu limitations And Click Edit limitation insert TransactionType خرید
	Then The result Should be insert Time 
	Examples:
	| Title |Symbolname| Massagefarsi |
	| معامله با تعداد مشخص شده ،به دلیل جهت معامله در بازار اعمال نمی شود | خساپا| اعمال محدودیت جهت معامله در نماد خساپا|
...
Scenario:  02)Buy stock symbol  with a minimum amount and a minimum number of day
	...
	Then the result should Buy shares of the symbol Message <Massagefarsi>
		Examples:
	 | Massagefarsi                      |
	 | اعمال محدودیت جهت معامله در نماد خساپا |
